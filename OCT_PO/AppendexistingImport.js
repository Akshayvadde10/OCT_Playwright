...
class AppendexistingImport {
    ...
    async appendExistingImport(ImportName, DatasetName,ImportType,Importpath) {
 ...
 await this.page.waitForTimeout(2000);
  ...
  await this.frame.locator('#AppendExistingId').click(); // Select Append to existing import option
...
await this.frame.getByRole('columnheader', { name: 'Destination' }).click(); // Close dropdown
  ...
  await this.frame.getByRole('columnheader', { name: 'Destination' }).click();
